...
/**
  * Description：基站案例之Spark SQL版演示，运行速度调查<br/>
...
object LacSparkSQLVersion {
  def main(args: Array[String]): Unit = {
    ...
    val spark: SparkSession = SparkSession.builder()
      .master("local[*]")
      .appName(LacSparkSQLVersion.getClass.getSimpleName)
      .getOrCreate()
    ...
    println(s"spark sql运行花费的时间是：${endTime - beginTime}ms")


    //spark sql运行花费的时间是：27051ms ← 第一次
    //spark sql运行花费的时间是：26577ms  ← 第二次
    //spark sql运行花费的时间是：29614ms ← 第三次
  }
}
